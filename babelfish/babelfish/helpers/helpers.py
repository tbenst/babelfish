...
def resize_tiff_3d(images, fx, fy, interpolation=cv2.INTER_LINEAR, out="ndarray"):
    ...
    print("no segfault")
    if out == "ndarray":
        new = np.zeros([images.shape[0], im.shape[0], im.shape[1]], dtype=np.float32)
    elif out == "memmap":
        new = np.memmap("babelfish_temp_memmap.mmap", np.float32, "w+", (images.shape[0], im.shape[0], im.shape[1]))
    new[0] = im
...
def resize_4d(images, fx, fy, interpolation=cv2.INTER_LINEAR, out="ndarray", is_tiff=False):
    im = cv2.resize((images[0, 0]), None, fx=fx, fy=fy, interpolation=interpolation)
    if out == "ndarray":
        new = np.zeros([images.shape[0], images.shape[1], im.shape[0], im.shape[1]], dtype=np.float32)
    elif out == "memmap":
        new = np.memmap(
            "babelfish_temp_memmap.mmap", np.float32, "w+",
...
def train_test_split_stim(stim_start_indices, stim_end_indices, fraction_in_train=0.8):
    """Doing random split to be more in line with `train_test_split`, could/should do start and end
    instead (which would be closer to actual "generalization" since data is non-stationary)

    Notes
    -----
    Need to be careful about always having np seeded

    ***Should take in context to prevent going into next stim***
    """
    N = len(stim_start_indices)
    split_index = int(fraction_in_train * N)
    indices = np.random.permutation(N)
    train_start_end = [(stim_start_indices[i], stim_end_indices[i]) for i in indices[:split_index]]
    test_start_end = [(stim_start_indices[i], stim_end_indices[i]) for i in indices[split_index:]]
    return train_start_end, test_start_end
...
def assign_tiff_Z_to_dset(dset, tiff, Z=0, batch_size=1024, progress=False):
    ...
    tiff_shape = tiff.series[0].shape
    if progress:
        gen = tqdm(range(0, tiff_shape[0], batch_size), total=int(tiff_shape[0] / batch_size))
    else:
